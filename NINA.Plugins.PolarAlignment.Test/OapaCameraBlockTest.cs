...
namespace NINA.Plugins.PolarAlignment.Test {
    ...
    public class OapaCameraBlockTest {
        ...
        [Test]
        public async Task Calibration_CameraBusy_KeepsThePreviousUnappliedResult() {
            // The start withdraws the previous result before the camera is checked, so that
            // Apply cannot run while a pass is measuring. A start that is then refused has
            // measured nothing, and must not silently cost the user the result they had.
            var (vm, _, camera) = Vm();
            ...
            vm.HasCalibrationResult.Should().BeTrue("a refused start replaces nothing");
            vm.DiscoveredXRatio.Should().Be(400f);
            vm.DiscoveredYRatio.Should().Be(200f);
        }
